...
export const createVideo = async (req: express.Request, res: express.Response) => {
    const userIdRawAuth = (req.auth as any)?.userId ?? (req.auth as any);
    const userId = Array.isArray(userIdRawAuth) ? userIdRawAuth[0] : userIdRawAuth;
    if (!userId) {
      return res.status(401).json({ message: "Unauthorized" });
...
export const getAllPublishedProjects = async (req: express.Request, res: express.Response) => {
...
export const publishProject = async (req: express.Request, res: express.Response) => {
    try{
        const userIdRawAuth = (req.auth as any)?.userId ?? (req.auth as any);
        const userId = Array.isArray(userIdRawAuth) ? userIdRawAuth[0] : userIdRawAuth;
        if(!userId){
        ...
        const projectId: string = Array.isArray(projectIdRaw) ? projectIdRaw[0] : projectIdRaw;
...
export const unpublishProject = async (req: express.Request, res: express.Response) => {
    try{
        const userIdRawAuth = (req.auth as any)?.userId ?? (req.auth as any);
        const userId = Array.isArray(userIdRawAuth) ? userIdRawAuth[0] : userIdRawAuth;
        if(!userId){
